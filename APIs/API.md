...
- This is the legacy editor for the user’s single primary org access role.
- Semantics are now “set the single primary org access template”.
- Passing more than one assignable org role id returns `400`.
...
- Supports the same `cursor` and `limit` pagination pattern as org conversations
- Each item returns:
  - `id`
  - `orgId`
  - `orgName`
