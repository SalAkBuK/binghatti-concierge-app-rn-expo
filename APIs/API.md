...
- Body: `{ name, email, password?, unitId }`
- Requires `residents.write`
- Creates User + ACTIVE Occupancy atomically
- Returns `{ userId, name, email, unit: { id, label }, buildingId, tempPassword?, mustChangePassword: true }`
...
- Body: `{ title, description?, attachments?: [{ fileName, mimeType, sizeBytes, url }] }`
- Uses resident ACTIVE occupancy to select building/unit

GET `/resident/requests`
- Lists requests created by the resident
