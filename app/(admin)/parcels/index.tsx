...
export default function ParcelsScreen() {
  ...
  const handleUpdateStatus = useCallback(async (parcelId: string, newStatus: Parcel["status"]) => {
    try {
  ...
  const handleNotifyTenants = useCallback(async () => {
    try {
      ...
      console.error("Failed to send parcel notifications:", error);
  ...
  const handleExportCSV = useCallback(async () => {
    try {
      ...
      console.error("Failed to export parcel CSV:", error);
  ...
  const renderItem = useCallback(({ item }: { item: Parcel }) => (
  ...
  ), [stats, allBuildings, selectedBuildingId, statusFilter, searchQuery, handleNotifyTenants, handleExportCSV]);
